...
> **Datum:** Decembar 2024
> **Projekat:** RabBooking
> **Status:** Production-Ready Architecture Plan (Updated)
...
## 6. Custom Domain Setup (Faza 2 - Kasnije)

### 6.1 Kad Kupiš Domenu

```
FAZA 2: CUSTOM DOMAIN (kad kupiš domenu)
Trajanje: 4-6 sati
Status: ČEKA KUPOVINU DOMENE

┌─────────────────────────────────────────────────────────────────┐
│  KORAK 1: Kupovina Domene (30 min)                              │
│  ──────────────────────────────────                             │
│  □ Odaberi ime (npr. bookingadria.hr, jadranbook.com, itd.)     │
│  □ Kupi kod registrara (Domenca, GoDaddy, Namecheap)            │
│                                                                 │
├─────────────────────────────────────────────────────────────────┤
│  KORAK 2: Cloud Run Proxy (2-3 sata)                            │
│  ────────────────────────────────                               │
│  □ Kreiraj Cloud Run service za wildcard routing                │
│  □ Deploy proxy service                                         │
│  □ Test sa jednim subdomenom                                    │
│                                                                 │
├─────────────────────────────────────────────────────────────────┤
│  KORAK 3: DNS Setup (1-2 sata)                                  │
│  ────────────────────────                                       │
│  □ A record: @ → Cloud Run IP                                   │
│  □ CNAME: * → Cloud Run domain                                  │
│  □ Čekaj DNS propagaciju (do 48h)                               │
│                                                                 │
├─────────────────────────────────────────────────────────────────┤
│  KORAK 4: Update Environment (30 min)                           │
│  ─────────────────────────────────                              │
│  □ Set BOOKING_DOMAIN env variable                              │
...
### 6.2 Cloud Run Proxy Service

```typescript
// cloudrun/subdomain-proxy/index.ts

import express from 'express';
import { createProxyMiddleware } from 'http-proxy-middleware';

const app = express();
const WIDGET_URL = 'https://rab-booking-widget.web.app';

app.use('/', (req, res, next) => {
  const host = req.hostname;
  const subdomain = extractSubdomain(host);

  // Proxy to Firebase Hosting with subdomain as query param
  createProxyMiddleware({
    target: WIDGET_URL,
    changeOrigin: true,
    pathRewrite: (path) => {
      const url = new URL(path, WIDGET_URL);
...
### 6.3 DNS Records (Kad Budeš Spreman)

```
# Primjer za {brandname}.hr

TYPE    NAME    VALUE                              TTL
───────────────────────────────────────────────────────
A       @       [Cloud Run IP - dobit ćeš nakon deploy-a]
CNAME   *       [Cloud Run Domain - xxx.run.app]
CNAME   www     {brandname}.hr
```
...
```typescript
// Auto-generate subdomain on property create
export const onPropertyCreate = onDocumentCreated(
  'properties/{propertyId}',
  async (event) => {
    const property = event.data?.data();
    if (!property || property.subdomain) return;

    const subdomain = await generateUniqueSubdomain(property.name);

    await event.data?.ref.update({ subdomain });

    logInfo('Auto-generated subdomain', {
...
### Faza 1 (Bez Domene) - Može Odmah

- [ ] Property model update sa subdomain field
- [ ] PropertyBranding model
- [ ] checkSubdomainAvailability Cloud Function
- [ ] generateSubdomainFromName Cloud Function
- [ ] Update emailService.ts
- [ ] SubdomainService (widget)
- [ ] BookingViewScreen branding support
- [ ] SubdomainNotFoundScreen
- [ ] Owner Dashboard subdomain UI
- [ ] Unit tests
- [ ] Integration tests
- [ ] Manual testing

### Faza 2 (Sa Domenom) - Čeka Kupovinu

- [ ] Kupovina domene
- [ ] Cloud Run proxy setup
- [ ] DNS configuration
- [ ] BOOKING_DOMAIN env variable
- [ ] End-to-end test sa pravom domenom
