...
- 🚨 [audit/92-cf-smoke-payment-2026-05-30.md](./audit/92-cf-smoke-payment-2026-05-30.md) - Payment + booking CF smoke on bookbed-dev (51 cases, 9 CFs, NULL PROD writes). **F-92-02 P1**: `findBookingById` Strategy 2 builds bookingChecks against `properties/*/units/*/bookings/{id}` (4-level legacy) but canonical write target is `properties/*/bookings/{id}` (3-level, `atomicBooking.ts:1216`) → `guestCancelBooking` + `updateBookingTokenExpiration` (the 2 callers without ownerId Strategy-1 hint) silently NOT_FOUND every legit booking. Fixed in-PR: parallel-check both paths + `unitId ?? data.unit_id` fallback; 10/10 jest green incl. new canonical-path regression. Other findings: F-92-01 P3 `BASE_ALLOWED_DOMAINS` includes `http://localhost` for ALL envs (PROD too); F-92-03 P3 webhook accepts GET/PUT; F-92-04 P3 webhook returns 500 (not 400) on malformed JSON. Verified: SF-001 ownerId server-fetch, SF-005 phone XSS strip, SF-008 notes 1000 cap, SF-022 Sentry HttpsError client-fault filter clean, SF-027 priceId fail-CLOSED (allowlist empty), SF-034/054 NO stack leak in 51 responses. Skipped: webhook dedup smoke (needs Stripe-signed event replay), Connect refund (blocked F-70-02 hCaptcha). Renumbered from 91 to avoid collision with parallel session [[sanitize-email-no-format-check]]. (2026-05-30)
