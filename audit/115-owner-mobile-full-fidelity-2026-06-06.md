...
1. **G-1 — Booking detail modal `Završeno` status pill rendered BLUE** (`#4A90D9` = imported-blue token visual match). Should be PURPLE `BBColor.statusCompleted = #6B4CE6` per handoff status mapping. Same drift class as audit/114 F1 mjesečni legend. PR #677 commit `375b873c` scope was limited to `month_calendar_screen.dart:937`; the booking-detail modal status pill (`booking_details_dialog*.dart`) still maps `completed → BBColor.status_imported` (visual evidence in r4-33). **P2 — single-token swap fix shape, identical to F1.**
...
| **P2** | **G-1 — Booking detail modal status pill `Završeno` purple** | Same single-token swap shape as F1 (`BBColor.statusCompleted`). Suspected site: `booking_details_dialog.dart` / `_v2.dart` status-pill builder. Surface: any post-checkout booking modal. | **NEW this audit** |
