...
## AI Assistant premium fidelity (2026-06-16, `ec78235b`)

Follow-up to §11 (which token-fixed the old screen). Full premium-fidelity pass to handoff `ai-assistant.jsx`. Single commit direct on `main`. AI is **LIVE Gemini** (`firebase_ai`) → shell styled only, no fabricated output; `_PregledAiInsight` (flagged placeholder) untouched (data-honesty). Operator scope calls: Q1 = match-handoff-flat bubbles/composer; Q2 = enrich active-chat header.

- **`ai_assistant_screen.dart`** (1173L): bubbles → flat (solid `primary` user / `surface`+border assistant, `BBRadius.xs` tail, 32px `AiBrandAvatar`, 11px timestamps from real `message.timestamp`, 70/78% max-width); `_buildMessageBubble` extracted to top-level `@visibleForTesting buildAiMessageBubble`; composer → bordered pill + solid-primary send + `aiAssistantDisclaimer` footer (attach omitted — no upload feature); empty/welcome radial hero + 5 chips + Image `errorBuilder`; consent **visual-only** restyle (icon tiles + eyebrow/H1 — `PopScope`/`_acceptConsent`/`grantAiChatConsent`/provider-gate **0 lines changed**); native `AlertDialog`→`BbDialog`; full token sweep (0 hex / 0 raw literals via named-const block, `AppShadows`→`BBShadow`, `colorScheme`→`BBColor`); `showTitle:false` on the 3 in-body-header `CommonAppBar` call-sites (consent appbar keeps its title).
- **`ai_assistant_premium_header.dart`**: added `AiBrandAvatar` (primary-tint r10 tile + `auto_awesome` fallback) + `AiConversationHeader` (avatar + title + status + copy/delete); kept landing `AiAssistantPremiumHeader`.
- **l10n:** +`aiAssistantCopyLast` / `aiAssistantMessageCopied` (en+hr); all other strings pre-existed.
- **Tests:** new `ai_assistant_premium_test` (12 overflow cells 360/414/768 × light/dark + header chrome + avatar fallback). Full suite **1475 pass**.
- **Live (Marionette, bookbed-dev):** desktop light+dark all states + **consent grant end-to-end** (logout→login→gate→accept→chats reappear) + logout robustness (real-tap → clean confirm dialog, no "Oops"; earlier ErrorBoundary was Marionette `scroll_to` — its own error text "…scroll attempts"). Mobile = test-proven; live-resize tooling-blocked (flutter's own Chrome can't be resized from here).
- **Seed:** `scripts/seed-ai-eyeball-dev.js` (untracked dev tool; `--clear` to remove the 2 seeded `ai_chats`).
- **Deferred:** owner PROD deploy — now **6 changes ahead** (Pregled + Rezervacije + Timeline + Mjesečni + global-chrome + AI assistant). Batch is over-ripe → next move = owner hosting-only PROD deploy + smoke.
