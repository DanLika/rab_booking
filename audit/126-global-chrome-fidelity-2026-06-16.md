...
## SHIPPED — §flatten (2026-06-16, CHANGELOG 7.23) — operator REVERSAL of 1A/1B

The 1A/1B ledger kept the **TIP-1 gray gradients** because they encoded the operator's then-standing preference. **Operator reversed it** → flat clean colors. This unlocked **Option 1A (drop gradient → flat = handoff)**, which `--bb-bg`/`--bb-panel-bg` always were. Two HALT gates (Phase-1 preview render, Phase-5 attestation); operator can't see agent renders, so the live app was the deliverable.

- **Complaint = exactly 2 consts:** `pageBackground` (TL→BR gray) + `sectionBackground` (TR→BL gray) — the two *opposite-direction* gray gradients. Both in `app_gradients.dart` → **one-file flip**.
- **Mechanism (Fork 1 = approved gray, not white):** make each gradient's two stops identical → flat fill, type stays `LinearGradient`, **all ~56 consumers unchanged**. 2-tone base/raised from the user's own curated consts: light shell `#ECEDF2` / raised `#FFF`+card `#FBFBFD`; dark shell `#1A1A1A` (NOT OLED `#000`) / raised+card `#2D2D2D`. **Dark dissolve-trap fixed:** card `#0B0B0D` was *darker* than the shell → cards sank; → `#2D2D2D` (one step above). 0 new hex.
- **Hero washes (Fork 2 = FLATTEN, operator-ruled from the live render):** the live AI-insight banner read as a visible purple→mint gradient, not a subtle accent. Flattened `_PregledAiInsight` (Pregled) + `bookings_premium_header` priority card (Rezervacije) → flat `c.surfaceVariant`; **purple icon tiles kept** as the only brand accent. Mint-wash signature (`mintWidget`/`#3DD9B0` low-alpha) grep = **0** in owner chrome after. **Kept** (different class): chart/progress data-viz fills, 4px amber rail, drag/drop, status/error chips, dialog headers, fade scrims.
- **Trial banner:** cream/amber + red gradients → flat tint; **EN→HR** ("Your trial ends in N days"→"Probno razdoblje istječe za N dana", expired + "Upgrade"→"Nadogradi"). Banner bypasses l10n (hardcoded) → l10n-key sweep flagged.
- **RenderFlex overflow (found during the live sweep, fixed):** Rezervacije `_Fact` chip Row had an unflexible `Text` → long property·unit name overflowed the Wrap inside the *constrained* priority card (`+114px @≈1352`, tripped ErrorBoundary "Oops"). Pre-existing (ledger responsive test pumps the *ledger*, not this header). Fix = `Flexible`+ellipsis + `@visibleForTesting buildBookingFactForTest` seam + `bookings_premium_header_fact_overflow_test` (8 widths × L/D = 16 cells).
- **Gate:** analyze 0 net-new (5 files clean) · format · full test **1495 pass** (+16) · `build web --no-tree-shake-icons` clean · grep 0 mint-wash · scope 5 lib + 1 test / **0 FROZEN**.
- **Live light (bookbed-dev):** Pregled = AI card flat + purple icon, trial banner flat + HR, flat `#ECEDF2` shell; Rezervacije = renders clean, long fact **ellipsizes**, header flat. **Dark = golden harness** (AI card flat on `#1A1A1A`, cards distinct). Anti-reversion: CLAUDE.md STANDARDI note + memory `flat-chrome-decision`.
