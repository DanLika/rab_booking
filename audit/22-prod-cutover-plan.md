...
- [ ] **`normalize-booking-nights.js` DRY-RUN against `rab-booking-248fc`**:
  ...
  - **DRY-RUN already performed 2026-05-23** (audit/22 follow-up): `K = 4` (4 bookings drift on `check_in`, same 4 on `check_out`). Scanned 10 confirmed-eligible bookings out of 58 total. All 4 drifters are summer-2026 confirmed bookings with check-in/check-out stored at 22:00 UTC of day N-1 (= midnight Zagreb CEST UTC+2) → normalize snaps to UTC midnight of the displayed civil day. Full log: `audit/migrations/2026-05-23-prod-sf026-normalize-DRYRUN.log`. A second dry-run inside the maintenance window is still wise (fresh count in case any new bookings arrive between now and cutover).
...
**Q4.** ~~Has SF-026's migration script been tested against any prod-shape data in dev…~~ **RESOLVED 2026-05-23** (audit/22 follow-up). Dev has only 1 booking (status `cancelled`, not eligible) so dev-side prod-shape validation is impossible. Instead ran the dry-run **directly against PROD** (read-only, no writes): script reports `Scanned: 10, Drifting check_in: 4, Drifting check_out: 4, Bookings to update: 4`. The 4 drifters are all summer-2026 confirmed bookings; pattern is `22:00:00.000Z day N-1 → 00:00:00.000Z day N` (current state is midnight Zagreb in CEST UTC+2 = 22:00 UTC of prev day; normalize snaps to UTC midnight of the displayed civil day, which is the new write-time invariant). Idempotency confirmed by definition (script writes only on drift; re-run on normalized rows updates nothing). Full log: `audit/migrations/2026-05-23-prod-sf026-normalize-DRYRUN.log`. **§3.6 prod `--force` is now well-characterized**: at most 4 doc updates, single batch, sub-second commit. Recovery, if needed, is per-booking using the pre/post ISO pairs in the log (and a second pre-flight dry-run inside the maintenance window will recapture against any newly-arrived bookings).
