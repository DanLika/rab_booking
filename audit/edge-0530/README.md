...
| `t2-overlap-race.js` | `createBookingAtomic` overlap race + turnover | live CF + **seed.js fixtures** | ✅ PASS (live) |
| `t3-availability.js` | `getUnitAvailability` inclusive-end edge | live CF + seed | ✅ PASS (live); F-86-01 inclusive-end window still reproduces as documented above |
| `t5-price.js` | SF-014 price server-authority | live CF + seed | ✅ PASS (live) — earlier "FAIL" was the missing fixture, resolved by re-seeding |
| `t6-stripe.js` | Stripe deposit / minimum / fee-allocation | live CF + seed | ✅ PASS (live) |
| `seed.js` | provisions `EDGE_prop_0530` fixtures | Admin SDK, dev-only | run 2026-06-11 ✓ (re-run before t2/t3/t5/t6) |
