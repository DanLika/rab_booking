...
| `audit123-deploy-smoke.js` | Post-deploy smoke for the audit/123 fix wave: (1) F-92-01 regression — live feed GET valid-token 200 VCALENDAR + wrong-token 403; (2) F-123-07 — signs in as test owner, hammers `getStripeAccountStatus` until `RESOURCE_EXHAUSTED` (expects ~call 31, limit 30/300s; in-memory L1 so a multi-instance split can stretch it). First run 2026-06-11 post dev-deploy: feed 200/403 ✓, exhausted at call 31 ✓. NOTE: burns the test owner's 30/300s budget — wait 5 min before re-running anything that calls that CF. | `GOOGLE_CLOUD_PROJECT=bookbed-dev node audit/smoke/audit123-deploy-smoke.js` (repo root; ADC + test account from memory) |
