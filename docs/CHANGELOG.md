...
**Last Updated**: 2026-05-23 | **Version**: 6.89

---

**Changelog 6.89**: `/vibe-security` audit → hotfix PR #462 (Critical role escalation + 3 others) + audit/30 candidate (2026-05-23):

- **`/vibe-security` slash command run** against current repo state. Surfaced **1 Critical + 2 High + 1 Medium** findings beyond audit/29's scope (audit/29 = parallel-session security audit focused on iCal SSRF redirect + resend-email DoS — disjoint surface from this run).
- **🔴 Critical — `role` field privilege escalation in firestore.rules.** The `isAdminFromFirestore()` rule helper trusts `users/{uid}.role == 'admin'`, but `role` was NOT in the protected-fields allowlist on either `users/{userId}` or `users/{userId}/data/{document}` update/create rules. Any authenticated user could self-promote to admin in one Firestore write (`users.doc(myUid).update({role: 'admin'})`) → satisfies `isAdminFromFirestore()` → DB-wide superuser. **Cloud Functions unaffected** — they gate on the JWT custom claim `request.auth.token.isAdmin`; the gap was rules-only.
- **🟠 High — Dormant-5 deletion clears `tsc` deploy block.** Branch `chore/delete-dormant-5-email-templates` deleted 5 template files but left re-exports in `functions/src/email/index.ts` (lines 53-82) → `npm run build` failed → blocked any CF deploy of the Critical fix above. Bundle deletes the 5 dangling exports + 5 orphan wrappers + imports/types in `emailService.ts` (per audit/28 §3.3 Option A; 0 external callers per audit/28 §3.1). Reminders continue via FCM push path (`scheduledPushNotifications.ts`); cancel/refund remain product gaps to wire later.
- **🟠 High — `ALLOWED_SUBSCRIPTION_PRICE_IDS` not enforced.** `stripeSubscription.ts:37-47` accepted any client-supplied `priceId` (allowlist commented as TODO). Attacker could pass any Stripe Price ID in the account (e.g. archived €0.01 test prices) → unintended subscription terms. Env var was already provisioned per `widget-secrets-exfil` prereqs but never consumed. Fix: deny-all (fail-CLOSED) when env empty → surfaces misconfig on first checkout attempt.
- **🟡 Medium — `Referrer-Policy: no-referrer` on `getUnitIcalFeed`.** Defense-in-depth on token-in-URL. Calendar UAs strip Referer; this protects browser-opened feeds from leaking the iCal token via outbound link Referer.
- **PR #462 opened** (`hotfix/role-escalation-deploy-unblock`, commit `898d2108` after rebase onto `dffaa0e3`). **+6 new Firestore-rules tests** in `functions/test/firestore_rules/users.test.ts`: self-write `role:'admin'` denied, self-write `role:'moderator'` denied (field-allowlist not value-allowlist), `role` in initial create denied, non-protected fields still allowed (regression guard), admin-JWT-claim write to other doc allowed, role in `/data/{doc}` subcollection denied. **Rules test count: 24 → 30** (exceeds 27+ target). Build: 0 tsc errors. `flutter analyze`: 0 issues. `npm test`: 161/165 (4 pre-existing stripeConnect message-string failures unchanged — baseline match, not regressions).
- **Bundled scope** because Critical fix requires CF deploy, and deploy requires the Dormant-5 cleanup. Otherwise the rules change ships but the CF deploy is blocked at tsc.
- **`audit/30` candidate written** — doc-only follow-up proposing to **drop `isAdminFromFirestore()` entirely**, leaving JWT custom claim as the sole admin signal. M-effort (~4–6h): inventory Admin Dashboard cross-user reads → migrate to callable CFs that gate on the claim → drop the rule helper + the `|| isAdminFromFirestore()` tail on every gated rule. Not blocking (PR #462 closes the exploitable hole); defense-in-depth posture upgrade.
- **Cross-repo work landed in same session** — LeadDataScraper PR #272 (`docs/phase-c-shipped-mark-2026-05-23`, label `documentation`) flips Phase C status markers in `docs/bookbed-crossover.md` (§3 recommended porting order + §4 LLM-safety table ❌→✅ + §5 Phase C heading SHIPPED) + the CLAUDE.md A→E mid-arrow update + `docs/pr-merge-plan.md` "Recommended re-prioritisation" line. Tracks bookbed PR #460 landing of `lib/core/utils/prompt_safety.dart` (Gemini `<UNTRUSTED_DATA>` fence) — landed 3 fence sites + 14-test corpus.
- **Branch-race recurrence (4th this session class).** Pre-PR-create `git push` succeeded on `docs/phase-c-shipped-mark-2026-05-23`; parallel terminal then swapped HEAD to `docs/live-pipeline-run-2026-05-23` before `gh pr create`. Caught by branch-name diagnostic. Recovery: `git checkout docs/phase-c-shipped-mark-...` + `--head` flag on `gh pr create`. Same multi-agent git race pattern as 6.81 / 6.83 / 6.86 / 6.87 — sistemski, ne per-repo. Also observed primary BookBed CWD branch swap (`chore/delete-dormant-5-email-templates` → `security/audit-31-ssrf-rate-limit`) mid-session; worked around by isolating doc edits to the `/tmp/bb-hotfix-g` worktree.
- **Stash-graveyard side-effect** — 10+ deep stash list with `wip-*` / `parallel-session-*` labels accumulating in LeadDataScraper. Cleanup not blocking; flagged for operator review.
- **CLAUDE.md index updated** — added audit/27 (was missing from index even though `dffaa0e3` committed it), audit/30 (new). audit/28 entry updated to flag DORMANT-5 (A2) → resolved by PR #462. Version bumped 7.5 → 7.6.
- **PR #462 deploy prereqs** (operator-verified before merge): (1) `ALLOWED_SUBSCRIPTION_PRICE_IDS` env set on both `bookbed-dev` and `rab-booking-248fc`; (2) post-rules-deploy smoke: signup new dev user, attempt `users.doc(uid).update({role: 'admin'})` from browser console → expect `PERMISSION_DENIED`; (3) `curl -I` against `getUnitIcalFeed` URL → expect `Referrer-Policy: no-referrer` header.
