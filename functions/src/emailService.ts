...
import {getTrialExpiringSoonTemplate} from "./email/templates/trial-expiring-soon";
import {getTrialExpiredTemplate} from "./email/templates/trial-expired";
...
const APP_NAME = () => "BookBed"; // Constant for app name
...
export async function sendTrialExpiringEmail(
  ...
  try {
    ...
    const upgradeUrl =
      `https://app.bookbed.io/subscription?utm_source=trial_warning&utm_medium=email&days=${daysRemaining}`;

    const {subject, html} = getTrialExpiringSoonTemplate(
      name,
      APP_NAME(),
      daysRemaining,
      upgradeUrl
...
export async function sendTrialExpiredEmail(
  ...
  try {
    ...
    const upgradeUrl =
      "https://app.bookbed.io/subscription?utm_source=trial_expired&utm_medium=email";

    const {subject, html} = getTrialExpiredTemplate(
      name,
      APP_NAME(),
      upgradeUrl
    );
