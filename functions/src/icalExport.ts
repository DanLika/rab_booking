...
import { calculateMinStayGapBlocks, truncateTime, BlockedRange } from "./utils/icalUtils";
...
export const getUnitIcalFeed = onRequest(async (request, response) => {
  ...
  try {
    ...
    // 7e. Map bookings for iCal generation
    ...
    // Calculate gap blocks based on min_stay_nights
    // NOTE: Echo detection now handles merged blocks via containment analysis,
    // so exporting gap blocks is safe and improves availability accuracy.
    const gapBlocks = calculateMinStayGapBlocks(bookings, icalEvents, blockedRanges, minStayNights);
    const allBlockedRanges = [...blockedRanges, ...gapBlocks];
