...
// Define the allowed Stripe Price IDs from environment configuration
const stripePriceMonthlyPro = defineString("STRIPE_PRICE_MONTHLY_PRO");
const stripePriceYearlyPro = defineString("STRIPE_PRICE_YEARLY_PRO");
...
export const createSubscriptionCheckoutSession = onCall(
  ...
  async (request) => {
    ...
    // Validate the requested Price ID against configured allowed values
    const allowedPrices = [
      stripePriceMonthlyPro.value(),
      stripePriceYearlyPro.value(),
    ];
    ...
    try {
      ...
      // Create new customer
        ...
        logInfo(
          `Using existing Stripe customer ${customerId} for user ${userId}`
        );
      }
      ...
      logSuccess(
        `Created subscription checkout session ${session.id} for user ${userId}`
      );
