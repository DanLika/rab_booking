 ...
 */
export const verifyBookingAccess = onCall(
  {cors: getCorsAllowlist()},
  async (request) => {
    // SECURITY: IP rate limiting to prevent brute-force (30 requests/hr per IP)
    ...
    const {bookingReference, email, accessToken} = data;
    ...
    try {
      ...
      // Guard against null guest_email (e.g. admin-created bookings w/o email)
      if (
        !booking.guest_email ||
      ...
      const bookingDetails = {
        ...
        createdAt: booking.created_at ?
          safeToDate(booking.created_at, "created_at").toISOString() :
          null,
