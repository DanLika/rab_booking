...
class DeepLinkService {
  ...
  Future<bool> handleDeepLink(String url, BuildContext context) async {
    try {
      ...
      // Handle web URLs (strictly validated)
  ...
  bool _handleAppDeepLink(Uri uri, BuildContext context) {
    ...
    switch (path) {
      case '/owner/calendar':
        ...
        if (unitId != null) {
          // FIX: Use Uri to safely construct URL and prevent parameter injection
          final safeUri = Uri(
            path: '/owner/calendar',
            queryParameters: {
              'unit': unitId,
  ...
  static String generateBookingComBlockUrl({
    ...
    // URLs to whitelisted domains are safe to be constructed like this,
    // but using Uri is always better practice.
    // However, the base domain is hardcoded and safe.
