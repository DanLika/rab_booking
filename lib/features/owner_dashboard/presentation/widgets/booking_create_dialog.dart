...
class _BookingCreateDialogState extends ConsumerState<BookingCreateDialog> {
  ...
  Future<void> _createBooking() async {
    ...
    if (!_formKey.currentState!.validate()) {
      LoggingService.logWarning('[BookingCreate] Form validation failed');
      return;
    }

    if (_selectedUnitId == null) {
      _showError(l10n.bookingCreateSelectUnitError);
      return;
    }

    // FIX: Verify user is authenticated before proceeding
    final authState = ref.read(enhancedAuthProvider);
    final ownerId = authState.firebaseUser?.uid;
    if (ownerId == null) {
      LoggingService.logWarning('[BookingCreate] User not authenticated');
      _showError(l10n.bookingCreateError);
      return;
    }
    ...
    try {
      LoggingService.logOperation('[BookingCreate] Creating booking...');

      // Get the selected unit to retrieve its propertyId
      ...
      // Normalize dates to midnight (strip time component) before sending to
      // the CF. The CF also normalizes via SF-026 Zagreb-civil-day UTC, but
      // sending midnight from the picker keeps the round-trip stable.
      ...
      // audit/26 PR-A: route through createOwnerBookingAtomic so the overlap
      // check runs inside a Firestore txn (no race between concurrent owner
      // writes) and SF-026 normalization is applied server-side.
      final callableService = ref.read(ownerBookingCallableServiceProvider);
      await callableService.createBooking(
        unitId: _selectedUnitId!,
        ...
        // User reached this line either because no conflicts were detected
        // OR because they confirmed "Continue Anyway" in the overlap dialog.
        allowOverlap: conflicts.isNotEmpty,
      );
      ...
      if (mounted) {
        Navigator.of(
          context,
        ).pop(_checkInDate); // Return check-in date for scroll
        ErrorDisplayUtils.showSuccessSnackBar(
          context,
          l10n.bookingCreateSuccess,
        );
      }
    } catch (e, stackTrace) {
      // FIXED: Log error for debugging
      unawaited(
        LoggingService.logError('[BookingCreate] Failed', e, stackTrace),
      );
      ...
      if (mounted) {
        ErrorDisplayUtils.showErrorSnackBar(
          context,
          e,
          userMessage: l10n.bookingCreateError,
        );
      }
    } finally {
      if (mounted) {
          ...
          builder: (dialogContext) => Dialog(
            ...
            child: Container(
              ...
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  // Header with warning/error color
                  Container(
                    height: ResponsiveDialogUtils.kHeaderHeight,
                    padding: EdgeInsets.symmetric(horizontal: headerPadding),
                    ...
                    child: Row(
                      children: [
                        ...
                        IconButton(
                          ...
                          tooltip: l10n.bookingCreateCancel,
                        ),
                      ],
                    ),
                  ),
                        ...
                        children: [
                          Text(
                            conflicts.length == 1
                                ...
                                : l10n.bookingCreateOverlapMultiple(
                                    conflicts.length,
                                  ),
                      ...
                      children: [
                        ...
                        Flexible(
                          child: FilledButton(
                            ...
                            child: Text(
                              l10n.bookingCreateContinueAnyway,
                              overflow: TextOverflow.ellipsis,
                            ),
                          ),
                        ),
