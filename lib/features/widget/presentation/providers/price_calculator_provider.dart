...
final bookingPriceProvider = FutureProvider.family<BookingPriceBreakdown?, (String, DateTime?, DateTime?)>((
  ...
  // Validate dates
  if (normalizedCheckOut.isBefore(normalizedCheckIn) || normalizedCheckOut.isAtSameMomentAs(normalizedCheckIn)) {
    return null;
  }
  ...
  final dailyPrices = await dailyPriceRepo.getPricesForDateRange(
    ...
    endDate: normalizedCheckOut.subtract(const Duration(days: 1)), // Exclude check-out day
  );
  ...
  while (current.isBefore(normalizedCheckOut)) {
    ...
    if (dailyPriceModel != null) {
      ...
      priceForNight = dailyPriceModel.getEffectivePrice(weekendDays: weekendDays);
    } else {
      ...
      priceForNight = (isWeekend && weekendBasePrice != null) ? weekendBasePrice : fallbackPrice;
    }
  ...
  final numberOfNights = DateNormalizer.nightsBetween(normalizedCheckIn, normalizedCheckOut);
