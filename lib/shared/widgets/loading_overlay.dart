...
class _LoadingOverlayState extends State<LoadingOverlay> {
  ...
  Widget build(BuildContext context) {
    ...
    // Use user provided color or default to semi-transparent black/white for glassmorphism
    final bgColor =
        widget.backgroundColor ??
        (isDarkMode
            ? Colors.black.withValues(alpha: 0.60) // Reduced opacity for blur visibility
            : Colors.white.withValues(alpha: 0.60));

    return BackdropFilter(
      filter: ImageFilter.blur(sigmaX: 5, sigmaY: 5), // Added blur
      child: Container(
        color: bgColor,
        child: Center(
          child: Column(
