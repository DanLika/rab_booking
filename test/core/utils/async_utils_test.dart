...
void main() {
  group('FutureTimeoutExtension', () {
    test(
      'withFirestoreTimeout completes successfully if future resolves before timeout',
      () {
        fakeAsync((async) {
          ...
          Future.delayed(
            ...
            () => 'success',
          ).withFirestoreTimeout().then((_) => completed = true);
    ...
    test(
      'withFirestoreTimeout throws TimeoutException if future does not resolve before timeout',
      () {
        fakeAsync((async) {
          Object? exception;
          ...
          Future.delayed(
            ...
            () => 'fail',
          ).withFirestoreTimeout().catchError((e) {
            exception = e;
            return 'error_caught';
          });
          ...
          expect(
            ...
            contains('Firestore query timed out after 30s'),
          );
        });
      },
    );
    ...
    test(
      'withFirestoreTimeout throws TimeoutException with custom operation name',
      () {
        fakeAsync((async) {
          Object? exception;
          ...
          Future.delayed(
            const Duration(seconds: 40),
            () => 'fail',
          ).withFirestoreTimeout('getUserProfile').catchError((e) {
            exception = e;
            return 'error_caught';
          });
          ...
          expect(
            (exception as TimeoutException).message,
            contains('getUserProfile timed out after 30s'),
          );
        });
      },
    );
    ...
    test('withHttpTimeout respects http timeout constant', () {
      fakeAsync((async) {
        ...
        Future.delayed(
          TimeoutConstants.httpRequest + const Duration(seconds: 1),
          () => 'fail',
        ).withHttpTimeout().catchError((e) {
          exception = e;
          return 'error_caught';
        });
    ...
    test(
      'withCloudFunctionTimeout respects cloud function timeout constant',
      () {
        fakeAsync((async) {
          Object? exception;

          Future.delayed(
            TimeoutConstants.cloudFunction + const Duration(seconds: 1),
            () => 'fail',
          ).withCloudFunctionTimeout().catchError((e) {
            exception = e;
            return 'error_caught';
          });
          ...
          expect(
            (exception as TimeoutException).message,
            contains('Cloud Function timed out'),
          );
        });
      },
    );
    ...
    test('withBookingFetchTimeout respects booking fetch timeout constant', () {
      fakeAsync((async) {
        ...
        Future.delayed(
          TimeoutConstants.bookingFetch + const Duration(seconds: 1),
          () => 'fail',
        ).withBookingFetchTimeout().catchError((e) {
          exception = e;
          return 'error_caught';
        });
        ...
        expect(
          (exception as TimeoutException).message,
          contains('Booking fetch timed out'),
        );
      });
    });
    ...
    test('withListFetchTimeout respects list fetch timeout constant', () {
      fakeAsync((async) {
        ...
        Future.delayed(
          TimeoutConstants.listFetch + const Duration(seconds: 1),
          () => 'fail',
        ).withListFetchTimeout().catchError((e) {
          exception = e;
          return 'error_caught';
        });
    ...
    test('withShortTimeout respects short operation timeout constant', () {
      fakeAsync((async) {
        ...
        Future.delayed(
          TimeoutConstants.shortOperation + const Duration(seconds: 1),
          () => 'fail',
        ).withShortTimeout().catchError((e) {
          exception = e;
          return 'error_caught';
        });
    ...
    test('withCustomTimeout respects given duration', () {
      fakeAsync((async) {
        ...
        Future.delayed(
          ...
          () => 'fail',
        ).withCustomTimeout(customDuration).catchError((e) {
          exception = e;
          return 'error_caught';
        });
        ...
        expect(
          (exception as TimeoutException).message,
          contains('Operation timed out after 42s'),
        );
      });
    });
  });
  ...
  group('StreamTimeoutExtension', () {
    ...
    test('firstWithTimeout throws if stream does not emit before timeout', () {
      fakeAsync((async) {
        ...
        expect(
          (exception as TimeoutException).message,
          contains('Stream timed out waiting for first value'),
        );
        controller.close();
      });
    });
    ...
    test('firstWithTimeout respects custom duration if provided', () {
      fakeAsync((async) {
        ...
        expect(
          (exception as TimeoutException).message,
          contains('Stream timed out waiting for first value after 42s'),
        );
        controller.close();
      });
    });
