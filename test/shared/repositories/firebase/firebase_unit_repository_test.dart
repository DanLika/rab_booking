...
void main() {
  group('FirebaseUnitRepository.fetchUnitByIdFresh', () {
    ...
    setUp(() {
      ...
      ).thenReturn(mockPropertyDoc);
      when(() => mockPropertyDoc.id).thenReturn('test-property-id');
      when(
        () => mockPropertyDoc.collection('units'),
      ).thenReturn(mockUnitsCollection);
      when(() => mockUnitsCollection.doc(any())).thenReturn(mockUnitDoc);
    });

    test(
      'should return unit when document exists and passes GetOptions(source: Source.server)',
      () async {
        ...
        verify(
          () => mockUnitDoc.get(const GetOptions(source: Source.server)),
        ).called(1);
    ...
    test(
      'should fall back to fetchUnitById when FirebaseException(unavailable) is thrown',
      () async {
        ...
        when(() => mockFirestore.collectionGroup('units')).thenReturn(
          mockUnitsCollection as dynamic,
        ); // collectionGroup returns Query, CollectionReference is a Query
        when(
          () => (mockUnitsCollection as dynamic).get(),
    ...
    test(
      'should rethrow FirebaseException if code is not unavailable',
      () async {
        when(() => mockUnitDoc.get(any())).thenThrow(
          FirebaseException(
